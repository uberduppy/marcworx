 ...
 * @author Todd
 */
public abstract class BaseMarcLeader implements IMarcObject {
    ...
    public BaseMarcLeader(char recordType) {
        preConstruction();
        populateLeader(recordType);
        postConstruction();
    }

    public BaseMarcLeader(String data) {
        preConstruction();
        assignData(data);
        postConstruction();
    }
    ...
    private void populateLeader(char recordType) {
        String leaderData = "00000" + RecordStatusConstants.RECORD_STATUS_NEW +
                recordType +
                DataTypeConstants.DATA_TYPE_UNKNOWN +
                "  2200000   4500";
